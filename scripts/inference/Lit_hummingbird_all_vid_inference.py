...
dirs = find_checkpoints(
    Path(f"{prefix}hummingbirds-pil"), version="3anp5naw", log="best"
)  # .glob("**/*.ckpt")) # Long Tr DA asym
...
print(
    f"Positive data dir: {model.pos_data_dir}, negative data dir: {model.neg_data_dir}"
)
...
for vname in videos[:]:
    ...
    dataloader = model.tst_external_dataloader(
        path=f"/data/shared/frame-diff-anomaly/data/{vname.name}/"
    )
    annot = pd.read_csv(
        f"{prefix}data/Weinstein2018MEE_ground_truth.csv"
    ).drop_duplicates()
    ...
    fpath_tdiff = Path(
        f"/data/shared/frame-diff-anomaly/data/{vname.name}/_scores_triplet.csv"
    )
    ...
    score_t_diff = (score_t_diff - score_t_diff.min()) / (
        score_t_diff.max() - score_t_diff.min()
    )
    ...
    df_sco = pd.DataFrame(
        index=range(len(pipeline_score)),
        columns=[
            "probability",
            "groundtruth",
            "score_pipeline",
