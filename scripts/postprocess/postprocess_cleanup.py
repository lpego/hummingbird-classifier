...
def delete_images_in_directory(
    ...
    optionally, checks whether images above a certain positive class probability threshold have been copied to an output folder.
    """
    ...
    print(f"######################### WARNING #########################")
    print(
        f"You're about to delete {len(image_files)} files, for a total of {size_str}; this is irreversible."
    )
    ...
    if input_csv and threshold is not None and out_folder:
        ...
        for image_path in filtered_df["image_path"]:
            ...
            if not (Path(out_folder) / filename).exists():
                print(
                    f"Warning: {filename} has not been copied to {out_folder}. Operation aborted."
                )
                return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Delete images based on model predictions."
    )
    parser.add_argument(
    ...
    exit(
        delete_images_in_directory(
            args.input_dir, args.input_csv, args.threshold, args.out_folder
        )
    )
