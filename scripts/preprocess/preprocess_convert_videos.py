...
from utils import cfg_to_arguments
...
if __name__ == "__main__":
    ...
    recoded_video_path = Path(
        "/data/shared/raw-video-import/data/RECODED_AnnotatedVideos/"
    )
    recoded_video_path.mkdir(exist_ok=True, parents=True)

    config = cfg_to_arguments(args.config)
    convert_videos(original_video_path, recoded_video_path, config.video_format)
