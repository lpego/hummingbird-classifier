...
torch.hub.set_dir("/data/shared/hummingbird-classifier/data")
...
# --- Difference score computation (as in test_torchcodec.py) ---
...
def preprocess_frame(frame, crop_size=None, imsize=None, do_filt=False, filt_rad=5):
...
def main():
    ...
    batch_size = 256  # Try 32, 64, 128, or higher if you have enough RAM/GPU

    # Optionally, prefetch frames in parallel using joblib for preprocessing
    ...
    #         frames_float.append(arr.astype(np.float32) / 255.0)
    #     return frames, frames_float
    ...
    #         delayed(preprocess_triplet)(triplet) for triplet in batch_triplets
    #     )
    #     input_tensors = [
    #         transform(Image.fromarray(frames[1])).unsqueeze(0)
    #         for frames, _ in processed
    #     ]
    #     triplet_frames_float = [frames_float for _, frames_float in processed]
    ...
    for batch_start in tqdm(
        ...
        for triplet in batch_triplets:
            ...
            input_tensors.append(transform(Image.fromarray(frames[1])).unsqueeze(0))
    ...
    print(df.head())
    df.to_csv("./system_test_fast_results.csv", index=False)
